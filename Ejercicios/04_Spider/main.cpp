...
int main()
{
    ...
    size_t szUltimaCoincidencia = html.text.find(patronInicial);

    while( szUltimaCoincidencia != string::npos )
    {
        szPosicionDeLaComilla = html.text.find(patronFinal, szUltimaCoincidencia + patronInicial.size());
        if( szPosicionDeLaComilla != string::npos )
        {
            ...
            cout << html.text.substr(inicioDelAtributo, f) << endl;
            contador++;
            szUltimaCoincidencia = html.text.find(patronInicial, szPosicionDeLaComilla + 1);
        }
    }
