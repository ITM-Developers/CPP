...
void __NOINLINE __libinteractive_init() {
	openstream(&__cave_in, "\\\\.\\pipe\\libinteractive_Main_cave_in", O_WRONLY);
	openstream(&__cave_out, "\\\\.\\pipe\\libinteractive_Main_cave_out", O_RDONLY);
}
...
static void __message_loop_cave(int __current_function, int __noreturn) {
	...
	while (readfull(&__cave_out, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x71651df7u: {
				// cave -> Main.tryCombination
				uint32_t __cookie;
				int tryCombination_N;
				readfull(&__cave_out, &tryCombination_N, sizeof(int), 1);
				int (*tryCombination_S) = (int(*))malloc(sizeof(int) * tryCombination_N);
				readfull(&__cave_out, tryCombination_S, sizeof(int) * tryCombination_N, 1);
				readfull(&__cave_out, &__cookie, sizeof(__cookie), 1);

				int result =
				tryCombination(tryCombination_N, tryCombination_S);
			...
			case 0x685d93e7u: {
				...
				int (*answer_S) = (int(*))malloc(sizeof(int) * answer_N);
				readfull(&__cave_out, answer_S, sizeof(int) * answer_N, 1);
				int (*answer_D) = (int(*))malloc(sizeof(int) * answer_N);
				readfull(&__cave_out, answer_D, sizeof(int) * answer_N, 1);
				readfull(&__cave_out, &__cookie, sizeof(__cookie), 1);

				answer(answer_N, answer_S, answer_D);
