...
static void __message_loop_Main(int __current_function, int __noreturn) {
	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				// Main -> cave.exploreCave
				uint32_t __cookie;
				int exploreCave_N;
				readfull(&__in, &exploreCave_N, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				exploreCave(exploreCave_N);
...
int tryCombination(int N, int S[]) {
	...
	writefull(&__out, S, sizeof(int) * (N));
	writefull(&__out, &__cookie, sizeof(__cookie));
	writeflush(&__out);
...
void answer(int N, int S[], int D[]) {
	...
	writefull(&__out, &N, sizeof(int));
	writefull(&__out, S, sizeof(int) * (N));
	writefull(&__out, D, sizeof(int) * (N));
	writefull(&__out, &__cookie, sizeof(__cookie));
	writeflush(&__out);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "\\\\.\\pipe\\libinteractive_cave_cave_in", O_RDONLY);
	openstream(&__out, "\\\\.\\pipe\\libinteractive_cave_cave_out", O_WRONLY);
